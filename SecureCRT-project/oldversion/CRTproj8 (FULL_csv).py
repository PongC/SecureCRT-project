...
import subprocess
import csv
#can connect and run in csv only (not parallel)
...
error_file = open("error.txt","wb+") #dummy variable
result_array=[]
...
def ssh(ip):
	...
	nIndex = 0
	nIndex = SCRIPT_TAB.Screen.WaitForStrings(wait_list,1)
	SCRIPT_TAB.Screen.Send(send_list[(nIndex-1)] + "\n")
	nIndex = 0
	nIndex = SCRIPT_TAB.Screen.WaitForStrings(wait_list,1)
	SCRIPT_TAB.Screen.Send(send_list[(nIndex-1)] + "\n")
...
def main():
	...
	csv_file = open(LOG_DIRECTORY+"/log.csv","wb+")
	worksheet = csv.writer(csv_file)
	worksheet.writerow(["IP","COMMANDS","OUTPUTS"])

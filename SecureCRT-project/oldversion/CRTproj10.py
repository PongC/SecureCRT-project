...
def ssh(ip):
	...
	wait_list=["dummy-dummy-dummy","assword:","(yes/no)?","<",">"]
	# wait_list=["djfhskjfhskjfhkf","aaaaaa"]
	...
	nIndex = 0
	nIndex = SCRIPT_TAB.Screen.WaitForStrings(wait_list,6)
	SCRIPT_TAB.Screen.Send(send_list[(nIndex-1)] + "\n")
	nIndex = 0
	...
	if(nIndex==0):
		...
		temp="LOGIN:Retry"+" ip: "+ip+ ""+"\r"
		worksheet.writerow([str(ip)])
		...
		if(nIndex==0):
			temp="LOGIN:RETRY ERROR"+" ip: "+ip+ "(can't login)"+"\r"
			worksheet.writerow(['','','',temp])
			return "login_error"
		else:
			temp="LOGIN:RETRY SUCCESS"+" ip: "+ip+ ""+"\r"
			worksheet.writerow(['','','',temp])
